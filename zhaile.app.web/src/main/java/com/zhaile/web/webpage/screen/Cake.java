...
public class Cake extends DefaultLayout {
	...
	public void execute(@Param("page") Integer page,
    	...
    	cache.getAdvertisementcache().click(cache.getAdvertisementcache().getAd(29).getId());
    	
    	context.put("menuN","29");
    	context.put("pageTarget","cake");
    	context.put("title", "富阳宅乐网-西式日韩店铺");
    	
    	Double lng = super.getLng();
		Double lat = super.getLat();
		if(page == null) page = 1;
    	
		Paging<ShopVO> shopList = shopManager.getShopVOByCateId(ShopCategoryEnum.西式日韩.getId(),lng,lat,page,10);
		
		context.put("shopList", shopList);
		context.put("ShopCategoryEnum", ShopCategoryEnum.getAll());
		context.put("SelectedCate", ShopCategoryEnum.西式日韩.getId());
    }
}
