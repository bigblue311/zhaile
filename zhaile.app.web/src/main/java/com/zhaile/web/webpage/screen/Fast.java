...
public class Fast extends DefaultLayout {
	...
	public void execute(@Param("page") Integer page,
    	...
    	cache.getAdvertisementcache().click(cache.getAdvertisementcache().getAd(27).getId());
    	
    	context.put("menuN","27");
    	context.put("pageTarget","fast");
    	context.put("title", "富阳宅乐网-中式简餐店铺");
    	
    	Double lng = super.getLng();
		Double lat = super.getLat();
		if(page == null) page = 1;
    	
		Paging<ShopVO> shopList = shopManager.getShopVOByCateId(ShopCategoryEnum.中式简餐.getId(),lng,lat,page,10);
		
		context.put("shopList", shopList);
		context.put("ShopCategoryEnum", ShopCategoryEnum.getAll());
		context.put("SelectedCate", ShopCategoryEnum.中式简餐.getId());
    }
}
